...
class InvitationsTest < ApplicationSystemTestCase
  ...
  # Accept invitation is an update action, which is not protected by default
  # (create-only scope), so no widget is injected on the accept form.
  test 'accept invitation page has no turnstile widget by default' do
